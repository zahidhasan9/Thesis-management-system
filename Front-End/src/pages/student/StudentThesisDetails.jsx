...
export default function StudentThesisDetails() {
  ...
  const resultReady = thesis.resultPublished === true && thesis.finalMarkStatus === "published" && typeof thesis.finalMark === "number";
  ...
  return <main className="min-h-screen bg-gray-50 px-4 py-8"><div className="mx-auto max-w-5xl space-y-6">
    ...
    <section className="grid gap-6 lg:grid-cols-[1fr_320px]">
      ...
      <aside className="rounded-lg border bg-white p-5 self-start"><div className="flex items-center gap-2"><GraduationCap size={20}/><h2 className="font-semibold">Final Result</h2></div>{resultReady ? <><p className="mt-6 text-5xl font-semibold text-gray-900">{thesis.finalMark}</p><p className="mt-2 text-sm text-emerald-700">Result Status: Published</p><p className="mt-1 text-xs text-gray-500">{thesis.resultPublishedAt ? new Date(thesis.resultPublishedAt).toLocaleString() : ""}</p></> : <><p className="mt-6 text-xl font-semibold text-gray-800">{progressLabel}</p><p className="mt-2 text-sm leading-6 text-gray-500">Your final mark will appear only after Admin approval and publication.</p></>}</aside>
    </section>
  </div></main>;
}
