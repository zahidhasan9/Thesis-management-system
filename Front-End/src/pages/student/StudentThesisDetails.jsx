...
export default function StudentThesisDetails() {
  ...
  return <main className="min-h-screen bg-gray-50 px-4 py-8"><div className="mx-auto max-w-5xl space-y-6">
    ...
    <section className="grid gap-6 lg:grid-cols-[1fr_320px]">
      ...
      <div className="rounded-lg border bg-white p-5"><h2 className="font-semibold">Evaluator Feedback</h2><div className="mt-4 space-y-4">{thesis.evaluatorFeedback?.length ? thesis.evaluatorFeedback.map((item) => <div key={`${item.position}-${item.submittedAt}`} className="rounded-lg bg-gray-50 p-4"><p className="text-xs font-medium text-gray-500">Evaluator {item.position}</p><p className="mt-2 whitespace-pre-line text-sm leading-6 text-gray-600">{item.feedback}</p>{item.submittedAt && <p className="mt-2 text-xs text-gray-400">Submitted: {new Date(item.submittedAt).toLocaleString()}</p>}</div>) : <p className="text-sm text-gray-500">No evaluator feedback available yet.</p>}</div></div>
